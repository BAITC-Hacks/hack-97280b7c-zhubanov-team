...
The backend returns `400` for a domain-invalid UTC issue time or a rolling range over 31 dates, `404` when an organizer CSV is missing, `422` for schema or model-data validation failures, and `503` when an eligible archived weather forecast is unavailable.
