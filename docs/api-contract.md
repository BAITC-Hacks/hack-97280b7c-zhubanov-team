...
`POST /api/forecast/explain` accepts `{"issue_time_utc":"2026-02-01T12:00:00Z","horizon_hours":48,"previous_issue_time_utc":"2026-01-31T12:00:00Z"}`; `previous_issue_time_utc` is optional and must be exactly one day earlier. The server recomputes the selected historical forecast(s) from its own inputs, prepares compact factual evidence, and returns `mode` (`ai` or `local`), Russian `text`, `notice` and `evidence`. `provider`/`model` are populated for a successful OpenAI or NVIDIA NIM response. OpenAI takes priority when both keys are configured. It never accepts user-supplied forecast values as evidence. With no key or a failed model request, the response remains available in `local` mode. This explanation is advisory and does not change the numeric forecast.
