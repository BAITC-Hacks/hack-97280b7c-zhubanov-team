...
- Подробное объяснение выбранного выпуска по проверяемым фактам: NVIDIA AI по кнопке при наличии ключа или локальный текст при недоступности сервиса.
...
| Необязательное объяснение | NVIDIA NIM, `meta/llama-3.3-70b-instruct` через серверный API |
...
| `backend/explanation.py` | Факты для объяснения, локальный текст и необязательный вызов NVIDIA |
...
Чтобы включить NVIDIA AI, сохраните секрет **локально** в `.env` в корне проекта:

```text
NVIDIA_API_KEY=<ваш ключ NVIDIA>
```

Также можно задать `NVIDIA_API_KEY` в окружении процесса перед запуском. Ключ не добавляйте в Git и не вставляйте в браузер. Вызов выполняется только по нажатию кнопки и расходует токены NVIDIA; без ключа или при ошибке сервиса появится помеченное **локальное объяснение**. Мы проверили формат запроса и переключение режимов тестами; в этой среде ключ NVIDIA не настроен, поэтому реальный ответ внешней модели здесь не проверен. Укажите ключ и перезапустите сервер перед проверкой через NVIDIA. Официальный [формат NVIDIA NIM](https://docs.api.nvidia.com/nim/reference/meta-llama-3_3-70b-instruct-infer) используется напрямую, без отдельной библиотеки.
...
5. Нажмите **«Объяснить этот выпуск»**: без NVIDIA-ключа появится подробное локальное объяснение с явной пометкой; с рабочим ключом — текст модели NVIDIA по тем же фактам. Числа прогноза и паспорт не изменяются.
