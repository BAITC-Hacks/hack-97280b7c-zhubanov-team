...
Это автоматизированный контур с фиксированной последовательностью действий. Контроллер `forecast.agent` принимает решения по правилам: новые входы → проверка → расчёт → проверка результата → публикация версии; неизменные входы → пропуск; ошибка → сохранение прошлого результата и повторная попытка. Codex использовался как AI-агент при разработке. В рабочем расчёте нет LLM-планировщика: прогноз выполняется Python-моделью, OpenAI/NVIDIA API и их ключи не требуются.
...
- [Презентация, речь и подготовка демо без интернета](docs/demo/README.md)
